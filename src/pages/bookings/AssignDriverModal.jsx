...
const AssignDriverModal = ({
  ...
  const handleSubmit = () => {
    ...
    console.log(drivers);
    // Fires off clean IDs directly to the backend API layer
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-4 py-2 border border-gray-200 dark:border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-yellow dark:bg-gray-800 text-gray-900 dark:text-white"
            >
              <option value="">Select a driver</option>
              {drivers.map((driver) => (
                /* ✅ Use driver.userId (or driver._id if that maps to your driver_profile.user_id uuid column) */
                <option key={driver._id} value={driver.id}>
          ...
          <div>
            <label className="block text-sm font-medium mb-1 flex items-center gap-2 text-gray-700 dark:text-gray-300">
              <HiOutlineTruck className="text-primary-yellow" /> Select Vehicle
              (Optional)
            </label>
            <select
            ...
            >
              ...
              {vehicles.map((vehicle) => (
                /* ✅ Pass structural database IDs as the underlying option context values */
                <option key={vehicle._id} value={vehicle._id || vehicle.id}>
                  {vehicle.company} - {vehicle.vehicleType}
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="flex justify-end gap-3 pt-4">
            ...
            <button
              ...
              className="px-4 py-2 bg-gradient-to-r from-primary-yellow to-yellow-600 text-primary-black rounded-lg font-semibold hover:shadow-lg transition-all"
            >
              Assign
            </button>
          </div>
