...
import { HiX, HiMail, HiPhone, HiCalendar, HiIdentification, HiCreditCard, HiLocationMarker, HiTruck, HiUser, HiBadgeCheck, HiDocumentText } from 'react-icons/hi';
...
const DriverDetailsModal = ({ isOpen, onClose, driver }) => {
  ...
  // Parse KYC documents if stored as JSON string
  let kycDocuments = {};
  ...
  // Helper to get image URL - Cloudinary URLs are already full
      ...
      <div className="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-3 text-center">
        ...
        {fullUrl ? (
          <img 
            ...
            onError={(e) => {
              console.error(`Failed to load ${title}:`, fullUrl);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${getStatusColor(details.verificationStatus)}`}>
                  {details.verificationStatus?.toUpperCase() || 'PENDING'}
                </span>
                <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${getAvailabilityColor(details.availabilityStatus)}`}>
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <InfoRow icon={<HiIdentification />} label="License Number" value={details.licenseNumber} />
          ...
          {/* Vehicle Information */}
          {(details.vehicleRegNumber || details.vehicleType || details.vehicleMake) && (
            <div>
              <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-3 flex items-center gap-2">
          ...
          <div>
            <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-3 flex items-center gap-2">
              <HiDocumentText className="text-primary-yellow" /> Documents
            </h3>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
              <DocumentCard title="Profile Photo" imageUrl={driver.image || kycDocuments.profilePhoto} />
              <DocumentCard title="Aadhar (Front)" imageUrl={kycDocuments.aadharFront} />
              <DocumentCard title="Aadhar (Back)" imageUrl={kycDocuments.aadharBack} />
              <DocumentCard title="PAN Card" imageUrl={kycDocuments.panImage} />
              <DocumentCard title="Driving License" imageUrl={kycDocuments.licenseImage} />
            </div>
          </div>
        </div>
